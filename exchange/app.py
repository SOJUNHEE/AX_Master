...
# -----------------------------------------------------------------------------
# 2. 커스텀 폰트 로드 및 와이드 레이아웃 극대화 CSS
# -----------------------------------------------------------------------------
...
custom_font_css += f"""
    ...
    .badge-export {{
        ...
        padding: 6px 12px; border-radius: 6px; font-weight: bold; font-size: 0.9rem;
        border: 1px solid #a7f3d0; margin: 3px;
    }}
    .badge-import {{
        display: inline-block;
        background-color: #eff6ff; color: #1e40af;
        padding: 6px 12px; border-radius: 6px; font-weight: bold; font-size: 0.9rem;
        border: 1px solid #bfdbfe; margin: 3px;
    }}
    .report-card {{
...
# =============================================================================
# [SECTION 1] 💱 와이드형 실시간 다중 통화 외환 정산 시뮬레이터 (핵심 환율 계산기)
# =============================================================================
...
with st.container(border=True):
    col_sel, col_val, col_quick = st.columns([1.5, 2.2, 3.3], gap="large")
    ...
    with col_quick:
        st.write("**신속 산출 프리셋 (Nominal Presets)**")
        q_cols = st.columns(4)
...
# 주요 G4 통화 와이드 카드 표시 (넓게 3분할)
display_cards = [c for c in ["USD", "KRW", "JPY", "EUR"] if c != base_currency][:3]
if len(display_cards) < 3:
    display_cards.append("CNY")

st.markdown(f"#### 📊 **{input_amount:,.2f} {base_currency}** 기준 주요 통화 환산 결제 대금 (Spot Settlement)")

card_cols = st.columns(len(display_cards), gap="medium")
for idx, c_code in enumerate(display_cards):
    ...
    with card_cols[idx]:
        with st.container(border=True):
            ...
            st.caption(f"재정환율: 1 {base_currency} = {item['단위당_기준환율']:,.4f} {c_code}")

with st.expander("📋 전 세계 주요 10대 교역 통화 실시간 고시 매트릭스 전체보기 (클릭하여 펼치기)", expanded=True):
    table_rows = []
...
# =============================================================================
# [SECTION 2] 🌟 와이드 대형 환율 전광판 & 외환 밸류에이션 진단
# =============================================================================
...
if position_pct >= 75:
    ...
    status_comment = "역사적 상단 밴드(75% 초과)에 위치하여 대외 통화 가치가 고평가된 국면입니다. 수출 채권 조기 회수 및 환차익 실현에는 최적기이나, 원자재 직수입 기업은 조달 단가 상승(Cost-Push) 리스크에 노출됩니다."
    ...
    status_badge_class = "badge-undervalued"
    status_comment = "역사적 지지선 하단(25% 미만)에 도달한 극심한 저평가 구간입니다. 해외 자산 취득 및 수입 부품 장기 물량 선도 계약(Forward Buy)의 전략적 매수 구간입니다."
...
<div class="hero-ticker-box">
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.15); padding-bottom: 14px; margin-bottom: 20px;">
        <div style="font-size: 1.3rem; font-weight: 700; color: #f8fafc;">
            {CURRENCY_INFO[target_currency]['flag']} {CURRENCY_INFO[target_currency]['name']} ({target_currency}) 실시간 시장 고시가
        </div>
        <div>
            <span class="hero-badge {status_badge_class}">밸류에이션: {status_label}</span>
        </div>
    </div>
    <div style="display: grid; grid-template-columns: 1.5fr 1.2fr 2.3fr; gap: 30px; align-items: center;">
        <div>
            <div style="font-size: 1.05rem; color: #94a3b8; font-weight: 600;">🇰🇷 원화 대비 환율 (1 {target_currency}{f'[{unit}]' if unit > 1 else ''} 당)</div>
            <div class="hero-rate-value">₩{current_krw_rate:,.2f}</div>
            <div style="font-size: 0.95rem; color: #38bdf8; font-weight: 600;">역사적 밴드 내 상위 {position_pct:.1f}% 위치</div>
        </div>
        <div style="border-left: 1px solid rgba(255,255,255,0.15); padding-left: 25px;">
            <div style="font-size: 1.05rem; color: #94a3b8; font-weight: 600;">🇺🇸 기축통화 대비 환율 (1 USD 당)</div>
            <div class="hero-sub-rate">{cur_to_usd:,.4f} {target_currency}</div>
            <div style="font-size: 0.9rem; color: #cbd5e1;">(기준 USD/KRW: ₩{usd_to_krw:,.1f})</div>
        </div>
        <div style="background: rgba(255,255,255,0.06); padding: 18px 22px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);">
            <div style="font-size: 0.95rem; color: #38bdf8; font-weight: 700; margin-bottom: 6px;">📌 환율 밸류에이션 총평</div>
            <div style="font-size: 0.95rem; color: #e2e8f0; line-height: 1.6;">{status_comment}</div>
        </div>
    </div>
</div>
...
if "30영업일" in time_mode:
    ...
    fig_time.add_trace(
        go.Scatter(x=df_daily["Date"], y=df_daily["Rate"], mode="lines+markers", name="일별 종가 환율 (KRW)", line=dict(color="#2563eb", width=3)),
        secondary_y=False
    )
    fig_time.add_trace(
        go.Bar(x=df_daily["Date"], y=df_daily["Change_Pct"], name="일일 등락률 (Day-to-Day %)", marker_color=np.where(df_daily["Change_Pct"] >= 0, '#ef4444', '#3b82f6'), opacity=0.45),
        secondary_y=True
    )
    ...
    fig_time = make_subplots(specs=[[{"secondary_y": True}]])
    fig_time.add_trace(
        go.Scatter(x=df_quarterly["Quarter"], y=df_quarterly["Rate"], mode="lines+markers+text", text=[f"{v:,.1f}" for v in df_quarterly["Rate"]], textposition="top center", name="분기별 마감 환율 (KRW)", line=dict(color="#10b981", width=3.5)),
        secondary_y=False
    )
    fig_time.add_trace(
    ...
    fig_time = make_subplots(specs=[[{"secondary_y": True}]])
    fig_time.add_trace(
        go.Scatter(x=df_yearly["Year"], y=df_yearly["Rate"], mode="lines+markers+text", text=[f"{v:,.1f}" for v in df_yearly["Rate"]], textposition="top center", name="연간 산술평균 환율 (KRW)", line=dict(color="#8b5cf6", width=3.5)),
        secondary_y=False
    )
    fig_time.add_trace(
...
# =============================================================================
# [SECTION 3] 📑 넉넉한 와이드형 對韓 대외 경상 교역 팩트시트
# =============================================================================
...
# 넉넉한 3열 지표 (글자 잘림 방지)
m_b1, m_b2, m_b3 = st.columns(3, gap="large")
...
with st.container(border=True):
    ...
    with p1:
        ...
        st.markdown(f"**수출 주력 품목군 (HS 2단위 기반):**<br>{export_badges}", unsafe_allow_html=True)
    with p2:
        import_badges = " ".join([f'<span class="badge-import">✓ {x}</span>' for x in trade_info['top_imports']])
        st.markdown(f"**수입 전략 조달 품목군 (HS 2단위 기반):**<br>{import_badges}", unsafe_allow_html=True)

# 넉넉한 2단 탭: 수출 vs 수입 실무 지침
...
with tab_tactics_ex:
    ...
    <div class="report-card">
        <h4 style="color: #1e3a8a; margin-top: 0;">🎯 수출 가격 경쟁력 및 결제 통화 포지셔닝</h4>
        <ul style="line-height: 2.0; color: #334155; font-size: 1.0rem;">
...
with tab_tactics_im:
    st.markdown(f"""
    <div class="report-card">
        <h4 style="color: #1e3a8a; margin-top: 0;">🏭 원자재 및 부품 조달 원가 방어 전략</h4>
        <ul style="line-height: 2.0; color: #334155; font-size: 1.0rem;">
